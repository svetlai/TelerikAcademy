namespace SubsetKWithSumS
{
    ...
    public class SubsetKWithSumS
    {
        ...
        public static void Main()
        {
            ...
            DisplayExample(array, k, sum, currentSubset, result);
        }
        ...
        private static void DisplayExample(int[] input, int k, int sum, int[] currentSubset, string result)
        {
            StringBuilder print = new StringBuilder();
            string border = new string('-', 60);

            print.AppendLine("Problem 17.* Subset K with sum S \nWrite a program that reads three integer numbers `N`, `K` and `S` and an array of `N` elements from the console. \nFind in the array a subset of `K` elements that have sum `S` or indicate about its absence.");

            // display examples
            print.AppendLine("Example:")
                .AppendLine(border)
                .AppendLine(string.Format("{0,3} | {1,30} | {2,3} | {3,3} | {4,3}", "N", "input", "K", "S", "result"))
                .AppendLine(string.Format("{0,3} | {1,30} | {2,3} | {3,3} | {4,3}", input.Length, string.Join(" ", input), k, sum, result))
                .AppendLine(border);
            ...
            print.Clear()
                .AppendLine(border)
                .AppendLine(string.Format("{0,3} | {1,30} | {2,3} | {3,3} | {4,3}", n, string.Join(" ", input), k, sum, result))
                .AppendLine(border);
